...
def index():
    ...
    pages_per_page =app.config['POSTS_PER_PAGE']
    posts = current_user.followed_posts().paginate(page,pages_per_page, False)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not next_page or url_parse(next_page).netloc != '': #parse url to check for domain
